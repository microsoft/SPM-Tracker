...
try:
    from . import trax_py3 as trax
    from .trax_py3 import server as trax_server
    TRAX = True
except ImportError:
    print('Cannot import TraX')
    TRAX = False
    raise ImportError
...
class VOT(object):
    ...
    def __init__(self, region_format):
        ...
        if TRAX:
            options = trax_server.ServerOptions(region_format, trax.image.PATH)
            self._trax = trax_server.Server(options)
