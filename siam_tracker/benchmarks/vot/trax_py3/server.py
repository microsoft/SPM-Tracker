...
from . import region as trax_region
from . import image as trax_image

# import trax_py3.region 
# import trax_py3.image
...
class Server(MessageParser):
    ...
    def wait(self):
        ...
        if message.type == None:
            ...
            image = trax_image.parse(message.arguments[0])
            region = trax_region.parse(message.arguments[1])
    ...
    def status(self, region, properties=None):
        """ Reply to client with a status region and optional properties.
            ...
            :param trax_py3.region.Region region: Resulting region object.
            :param dict properties: Optional arguments as a dictionary.
        """
        assert(isinstance(region, trax_region.Region))
...
class ServerOptions(object):
    ...
    def __init__(self, region, image, name=None, identifier=None):
        ...
        for r in region:
            assert(r in [trax_region.RECTANGLE, trax_region.POLYGON])
        for i in image:
            assert(i in [trax_image.PATH, trax_image.URL, trax_image.MEMORY, trax_image.BUFFER])
