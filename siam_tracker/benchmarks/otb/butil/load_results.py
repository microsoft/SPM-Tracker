...
def save_seq_result(result, meta_data=None):
    ...
    try:
        ...
        logging.error(list(map(type, result[0].__dict__.values())))
        sys.exit()
    ...
    if meta_data is not None:
        fileName = src + '/{0}.metadata.pickle'.format(seqName)
        resultFile = open(fileName, 'wb')
        pickle.dump(meta_data, resultFile)
        resultFile.close()
...
def load_result(evalType, tracker):
    ...
    logging.info(('Loading \'{0}\'...'.format(tracker)) + ('({0} seqs)'.format(len(resultNames) - 1)))
    return results, attrs

def load_seq_result(evalType, tracker, sequence):
    resultSRC = RESULT_SRC.format(evalType)
    logging.info('Loading {0}/{1}...'.format(tracker, sequence))
    src = os.path.join(resultSRC, tracker)
...
def load_scores(evalType, tracker, testname):
    resultSRC = RESULT_SRC.format(evalType)
    logging.info('Loading \'{0}\'...'.format(tracker))
