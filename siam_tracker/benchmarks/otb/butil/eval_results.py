            ...
            if evalType == 'TRE':
                if len(seq.gtRect) < result.endFrame:
                    ...
                    anno = seq.gtRect[result.startFrame-1:
                        result.endFrame]
            if evalType == 'OPE' and len(anno) != result.endFrame - result.startFrame + 1:
                anno = anno[result.startFrame-1:result.endFrame]
            logging.info('{0} : eval {1}'.format(tracker, seq.name))
            aveCoverage, aveErrCenter, errCoverage, errCenter = \
                calc_seq_err_robust(result, anno)
